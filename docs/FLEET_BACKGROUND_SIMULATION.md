...
**Implemented (8.1):** `BackgroundFleetSimulator` in `game_manager.js`; physics in `fleet_manager.updateFleetPhysics()`; ~12 fps background, 60 fps in space scene.
...
- Active ship: controlled in `space_scene` (also updated in loop when in space)
